...
func CompileRestoreScript(rs rules.RuleSet, pp system.PublishedPorts, geoFiles map[string]string, extraBypass ...string) string {
	...
	// The restore documents are staged in mktemp files. Under set -eu a
	// failed --test aborts the script before the explicit rm -f lines run,
	// so without this trap every rejected ruleset left a temp file behind.
	b.WriteString(`trap 'rm -f "${T4:-}" "${T6:-}"' EXIT` + "\n")
...
func zfwInRules(rs rules.RuleSet, rl []rules.Rule, dockerPorts map[int]bool, extraBypass []string) []string {
	out := []string{
		...
		// SYNs to the host's Zima Net address on 9527 are logged by ZFW-IN-DROP and discarded (125 in
